...
tmp <- expand.grid(mun = dpop_cut_age[, unique(mun)],
                   gender = dpop_cut_age[, unique(gender)], 
                   age = dpop_cut_age[, unique(age)],
                   year = dpop_cut_age[, unique(year)]
                   )
...
# TODO Andre why are there missing pop sizes?


# hack for now:
set(dpop, dpop[, which(is.na(pop_DANE))],'pop_DANE',0) 
set(dpop_cut_age, dpop_cut_age[, which(is.na(pop_DANE))],'pop_DANE',0) 
...
dref <- dm[ year == 2018, 
            list(natl_pop_n = sum(population)),
            by = c('gender','age')]
