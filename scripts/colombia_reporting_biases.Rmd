...
dmpi <- read.csv(file.path(data.dir,'COLOMBIA COVID and POP Datasets','multidimensional poverty','CNPV-2018_PobrezaMultidimensional_Municipal_IPM.csv'),
                 ...
                 nrow = 1122,
                 header = TRUE)[,1:6]
dmpi <- as.data.table(dmpi)
setnames(dmpi, 1:6, c('dep','mun','mun_name','mpi','mpi_cabeceras','mpi_centros'))
...
# Examine log-linear trends in age-specific deaths
...
```{r, include=TRUE, eval=TRUE, message=FALSE, echo=TRUE, warning=FALSE, tidy=TRUE}
# prepare national reference population
dref <- dm[ age != '0-9' & year > 2011 & year < 2022, 
            list(natl_pop_n = sum(pop),
                 natl_death_n = sum(deaths)
                 ),
            by = c('gender','age','year')]
dref[, natl_death_r := natl_death_n / natl_pop_n]
dref[, natl_death_r_log := log( natl_death_n / natl_pop_n )]

#determine age midpoints
tmp <- unique(subset(dref, select = c(gender, age)))
tmp[, age2 := age]
set(tmp, tmp[, which(age == '50+' & gender == 'Female')], 'age2', '50-67')
set(tmp, tmp[, which(age == '55+' & gender == 'Male')], 'age2', '55-77')
tmp[, age_from := as.integer(gsub('([0-9]+)-([0-9]+)','\\1', age2))]
tmp[, age_to := as.integer(gsub('([0-9]+)-([0-9]+)','\\2', age2))]
tmp[, age_mid := (age_from + age_to + 1L)/2]
dref <- merge(dref, 
              subset(tmp, select = c(age, gender, age_from, age_to, age_mid)), 
              by = c('age','gender')
              )
...
tmp <- dref[,
            {
              ...
              f <- gam( natl_death_r_log ~ s(age_mid, bs = "cs", k = 5), 
                        data = d, 
                        method = "REML"
                        )
              z <- mgcv:::predict.gam(f, d, type = 'response', se.fit = TRUE)
              list(natl_death_r_log_fitted = as.numeric(z$fit),
                   natl_death_r_log_fitted_se = as.numeric(z$se.fit),
                   age_mid = age_mid)
            },
            by = c('gender','year')]
dref <- merge(dref, tmp, by = c('year','gender','age_mid'))

# fit spline to age trends in death rates over continuous age mid points
tmp <- dref[,
            {
              d <- list( natl_death_r = natl_death_r, age_mid = age_mid)
              f <- gam( natl_death_r ~ s(age_mid, bs = "cs", k = 7), 
                        data = d, 
                        method = "REML"
                        )
              z <- mgcv:::predict.gam(f, d, type = 'response', se.fit = TRUE)
              list(natl_death_r_fitted = as.numeric(z$fit),
                   natl_death_r_fitted_se = as.numeric(z$se.fit),
                   age_mid = age_mid)
            },
            by = c('gender','year')]
dref <- merge(dref, tmp, by = c('year','gender','age_mid'))
...
p <- ggplot(dref, aes(x = age_mid, y = natl_death_r_log, colour = gender)) +
  ...
  labs(y = 'national log death rate')
ggsave(p, file = file.path(out.dir, paste0('Colombia_Log_Mortality_rate_curves_2011-2021_',stamp,'.png')), w = 10, h = 10)
...
p <- ggplot(dref, aes(x = age_mid, colour = factor(year), group = interaction(year, gender))) +
  ...
  facet_grid(~gender) +
  labs(y = 'national log death rate')
ggsave(p, file = file.path(out.dir, paste0('Colombia_Log_Mortality_rate_curves_2011-2021_gamfits_',stamp,'.png')), w = 10, h = 10)
...
p <- ggplot(dref, aes(x = age_mid, colour = factor(year), group = interaction(year, gender))) +
  ...
  theme_bw() +
  facet_grid(~gender) +
  labs(y = 'national death rate')
ggsave(p, file = file.path(out.dir, paste0('Colombia_Mortality_rate_curves_2011-2021_gamfits_',stamp,'.png')), w = 10, h = 10)
...
tmp <- da[, which(pop == 0.)]
set(da, tmp, 'mun_death_r_2018', 0.)
da <- subset(da, age != '0-9')
da[, mun_death_r_2018_log := log(mun_death_r_2018)]
...
tmp <- subset(dref, 
              ...
              select = c(gender, age, age_mid, natl_death_r_log_fitted, 
                         ...
                         natl_death_r_fitted_se) 
              )
da <- merge(da, tmp, by = c('gender','age'), all.x = TRUE)
...
da <- merge(da, tmp, by = c('mun'), all.x = TRUE)
...
p <- ggplot(tmp, aes(x = age_mid, colour = va, group = interaction(mun, gender))) +
  ...
  theme_bw() +
  facet_grid(~gender) +
  labs(y = 'log death rates\n in 30 municipalities with highest VA')
ggsave(p, file = file.path(out.dir, paste0('Colombia_Log_Mortality_rate_curves_2011-2021_mun_richest_',stamp,'.png')), w = 10, h = 10)
# --> essentially as expected around national curve
...
ggsave(p, file = file.path(out.dir, paste0('Colombia_Log_Mortality_rate_curves_2011-2021_mun_poorest_',stamp,'.png')), w = 10, h = 10)
# too many zeros, very sparse, so cannot see if below the national curve


da[, va_disc := cut(
  va, breaks = c(-1, 1e5, 5e5, 1e6, 1e10), 
  labels = c('<100,000','100,000-500,000','500,000-1,000,000','>1,000,000'), 
  right = FALSE)
  ]
...
p <- ggplot(da, aes(x = age_mid)) +
  ...
  labs(y = 'municipal log death rate', colour = 'valure aggregado\n in million pesos')
ggsave(p, file = file.path(out.dir, paste0('Colombia_Mortality_rate_curves_2011-2021_mun_by_va_',stamp,'.png')), w = 10, h = 10)
# clearly below on average

# Department capitals
tmp <- subset(da, capital )
p <- ggplot(tmp, aes(x = age_mid, colour = mun_name, group = interaction(mun, gender))) +
  ...
  theme_bw() +
  facet_grid(~gender) +
  labs(y = 'municipal log death rate')
ggsave(p, file = file.path(out.dir, paste0('Colombia_Log_Mortality_rate_curves_2011-2021_mun_richest_',stamp,'.png')), w = 10, h = 10)
```

# Analyze standardised death reates vs MPI by municipality 
...
# add MPI info
da <- merge(da, subset(dmpi, select = c(mun, mpi)), by = c('mun'), all.x = TRUE)
...
p <- ggplot(da, aes(x = mpi, y = death_r_std, col = factor(capital, levels = c(TRUE,FALSE), labels = c('state capital','other')))) +
  geom_point(data = subset(da, capital == FALSE)) +
  geom_point(data = subset(da, capital == TRUE)) +
  labs(col = '') + 
  scale_x_log10() +
  scale_colour_carto_d(palette = "Vivid", direction = -1) +
  theme_bw() +
  labs(x = 'multidimensional poverty index', y = 'mean standardized death rates 2017-2019') +
  facet_wrap(~dep_name, ncol = 6) +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1))
ggsave(p, file = file.path(out.dir, paste0('Colombia_Mortality_rate_2018_by_mpi_',stamp,'.png')), w = 10, h = 10)


p <- ggplot(da, aes(x = mpi, y = death_r_std, col = pop_r_disc)) +
  geom_point() +
  labs(col = '') + 
  scale_x_log10() +
  scale_colour_carto_d(palette = "Burg") +
  theme_bw() +
  labs(x = 'multidimensional poverty index', y = 'mean standardized death rates 2017-2019', colour = 'Population per km2') +
  facet_wrap(~dep_name, ncol = 6) +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1))
ggsave(p, file = file.path(out.dir, paste0('Colombia_Mortality_rate_2018_by_mpi2_',stamp,'.png')), w = 10, h = 10)


p <- ggplot(da, aes(x = mpi, y = death_r_std)) +
  geom_point(aes(col = pop_r_disc, 
                 pch = factor(capital, levels = c(TRUE,FALSE), labels = c('state capital','other')), 
                 size = factor(capital, levels = c(TRUE,FALSE), labels = c('state capital','other'))
                 )) +
  ...
  scale_colour_carto_d(palette = "Burg") +
  theme_bw() +
  labs(x = 'multidimensional poverty index', y = 'mean standardized death rates 2017-2019', colour = 'Population per km2', size = '', pch = '') +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) + 
  geom_smooth(colour = 'black') +
  geom_smooth(data = subset(da, capital), colour = 'black', lty = 'dotted', method = 'lm') 
ggsave(p, file = file.path(out.dir, paste0('Colombia_Mortality_rate_2018_by_mpi2_overall_',stamp,'.png')), w = 10, h = 10)



p <- ggplot(da, aes(x = mpi, y = pop_n/area_2, col = factor(capital, levels = c(TRUE,FALSE), labels = c('state capital','other')))) +
  geom_point(data = subset(da, capital == FALSE)) +
  geom_point(data = subset(da, capital == TRUE)) +
  labs(col = '') + 
  scale_x_log10() +
  ...
  scale_colour_carto_d(palette = "Vivid", direction = -1) +
  theme_bw() +
  labs(x = 'multidimensional poverty index', y = 'population density 2018 per km2') +
  facet_wrap(~dep_name, ncol = 6) +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1))
ggsave(p, file = file.path(out.dir, paste0('Colombia_Pop_density_2018_by_mpi_',stamp,'.png')), w = 10, h = 10)

library(splines)
summary(lm(death_r_std ~ bs(mpi) + bs(pop_r), data = da))
summary(lm(death_r_std ~ bs(mpi) + pop_r_disc, data = da))
